...
public class MessagesService {
  ...
  public StringBuilder getMessage(String initMessage, Exception... exceptions) {
    ...
    for (Exception exception : exceptions) {
      if (exception instanceof HttpClientErrorException && !((HttpClientErrorException) exception).getResponseBodyAsString().isEmpty()) {
        try {
          ...
          log.debug("Error while parsing an error", e1);
        }
      } else {
